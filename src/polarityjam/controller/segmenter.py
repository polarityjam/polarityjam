...
class CellposeSegmenter(Segmenter):
    ...
    def _get_cellpose_segmentation(self, im_seg, filepath):
        ...
        get_logger().info("Using model type '%s' with estimated cell diameter %s and flow threshold %s" % (
        self.params.model_type, self.params.estimated_cell_diameter, self.params.flow_threshold))
        ...
        if self.params.model_type == "custom":
            masks, flows, styles = model.eval(im_seg, diameter=self.params.estimated_cell_diameter,
                                              flow_threshold=self.params.flow_threshold, channels=channels)
        else:
