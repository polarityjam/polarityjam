...
class BioMedicalChannel:  # todo: make it a PIL image for enhanced compatability?
    ...
    def __init__(self, channel: np.ndarray):
        ...
        self.data = channel
        self.masks: Dict[
            str, Union[BioMedicalMask, BioMedicalInstanceSegmentation]
        ] = {}
...
class BioMedicalImage:
    ...
    def get_image_marker(self, img: np.ndarray):
        ...
        if self.img_params.channel_expression_marker >= 0:
            get_logger().info(
                ...
                % str(self.img_params.channel_expression_marker)
            )
            return BioMedicalChannel(
                img[:, :, self.img_params.channel_expression_marker]
            )
        return None
    ...
    def get_image_junction(self, img: np.ndarray):
        ...
        if self.img_params.channel_junction >= 0:
            get_logger().info(
                "Junction channel at position: %s"
                % str(self.img_params.channel_junction)
            )
    ...
    def get_image_nucleus(self, img: np.ndarray):
        ...
        if self.img_params.channel_nucleus >= 0:
            get_logger().info(
                "Nucleus channel at position: %s" % str(self.img_params.channel_nucleus)
            )
    ...
    def get_image_organelle(self, img: np.ndarray):
        ...
        if self.img_params.channel_organelle >= 0:
            get_logger().info(
                "Organelle channel at position: %s"
                % str(self.img_params.channel_organelle)
            )
